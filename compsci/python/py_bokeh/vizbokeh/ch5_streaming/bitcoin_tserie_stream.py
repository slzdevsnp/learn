...
def update():
    new_data=dict(x=[datetime.now(tz=timezone('Europe/London'))],y=[extract_value()])
    source.stream(new_data,rollover=200)
    print(source.data)
f.xaxis.formatter=DatetimeTickFormatter(
#milliseconds=["%Y-%m-%d-%H-%m-%S.%3Nms"],
seconds=["%Y-%m-%d-%H-%m-%S"],
minsec=["%Y-%m-%d-%H-%m-%S"],
minutes=["%Y-%m-%d-%H-%m-%S"],
hourmin=["%Y-%m-%d-%H-%m-%S"],
...
curdoc().add_root(f)
curdoc().add_periodic_callback(update,2000+randrange(1,100))
