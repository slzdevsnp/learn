...
def extract_value(market_name="bitstampUSD"):
    r=requests.get("http://bitcoincharts.com/markets/"+market_name+".html",
...
def update():
    new_data=dict(x=[datetime.now(tz=timezone('Europe/London'))],y=[extract_value(select.value)])
...
options=[("bitstampUSD","Bitstamp USD"),("krakenUSD","Kraken USD"),("coinsbankGBP","Coinsbank GBP")]
select=Select(title="Market Name",value="bitstampUSD",options=options)
