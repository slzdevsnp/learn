...
class TagsViewController : UITableViewController   {
    ...
    override func tableView(_ tableView: UITableView,
       ...
       let tag = tagDataSource.tags[indexPath.row]
        ...
        //usecase tapping on a row in a table view which already been selected
        if let index = selectedIndexPaths.index(of: indexPath){
            
            selectedIndexPaths.remove(at: index)
            photo.removeFromTags(tag)
        }else{  //tapping on a row which was not selected
        ...
        do {
            try store.persistenContainer.viewContext.save()
        } catch {
    ...
    @IBAction func addNewTag(_ sender: UIBarButtonItem) {
        ...
        //make a pop-up window to enter a new tag
        ...
        let okAction = UIAlertAction(title: "OK", style: .default) {
            ...
            if let tagName = alertController.textFields?.first?.text {
                let context = self.store.persistenContainer.viewContext
                let newTag =  NSEntityDescription.insertNewObject(forEntityName: "Tag",
                                                                  into: context )
                newTag.setValue(tagName, forKey: "name")
                
                do {
                    try self.store.persistenContainer.viewContext.save()
                } catch let error {
                    print("Core Data save failed: \(error)")
                }
                self.updateTags()
        ...
        let nokAction = UIAlertAction(title: "Cancel", style: .cancel,handler:nil)
