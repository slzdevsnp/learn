...
Now lets see the summary statistics for subjects of interest to our research question. Those are the individuals from a 'low' income category who do not have a health insurance plan and who are not in bad health.  
...
p1 <- ggplot(data = sel1_brfss2013 %>% filter(physhlth>=1 & physhlth <= 30) ,  aes(x = physhlth)) +
  geom_histogram(bins=8) +
  labs(x="number of days with bad health  in past 30 days") +
  ggtitle("Distribution of a number of sick days in a past month")
print(p1) 
```
We observe a bimodal distribution with peaks for 1 or 2 days  and 30 days. 


The healthy food is computed for 2 underlying variables. 
