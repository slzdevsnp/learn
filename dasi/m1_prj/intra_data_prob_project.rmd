...
**Research question 2:**
In this section we shall study a variable which represents for people a number of days with a poor physical health  in last 30 days. The dataset has  a variable on General health. But arguably the latter is more qualitative and  the former is more quantitative. Studying numbers of sick days that people take  has a direct economic interest reflecting a wealth expense rather than its generation by the employed population. We are interested to study factors which can potentially reduce the number of sick days taken. Among various factors we concentrate on  a physical activity levels and  on healthy food consumption. 
...
Now lets see the summary statistics for subjects of interest to our research question. Those are the individuals from a 'low' income category who do not have a health insurance plan and who are in good health. Because they are in good health they can theoretically afford  to bypass taking a health coverage plan.  
...
As with the previous question lets extract variables from a general dataset relevant to our current research question. The variable `X_frutsum` for a total of consumed fruits in a past month, `X_vegesum` a total of consumed vegetables in a past month, `X_pacat` a variable for a sport activity with 4 factor levels and `physhlth` is a number of days with bad physical health in a past 30 days. 
...
We observe a bimodal distribution with peaks for 1, 2 sick days  and 30 days. 


We can a median of sick days for different levels of physical activity 
...
The chart suggests   a dependency between the number of sick days and especially  an 'Inactive' level of physical activity.  It is interesting to observe a relatively high value of the median of sick days for people who did not reported their level of physical activity (NA category).  
...
The chart allow us to determine  a meaning cutoff of a variable (10 fruits or vegetables per day) to exclude bad data. For both food factors we observe  a unimodal right skewed distributions.
...
```
A contingency table for `is_sick` versus `is_sport_active` variables will help us to compute conditional probabilitiis such as a probability  to have at least one sick day given to maintain an active physical activity or to eat a healthy food. 
...
A probability to have at least 1 sick day and to have an active physical activitiy is
...
The join probability to have sick days and consume healthy food and a conditional probability to have sick days given a consumption of healthy food : 
...
Thus we first observe that both factors of healthy food and of active physical activity have a positive impact in reducing   the number of sick days.  It is also clear that the factor of active physical activity has a stronger positive impact in reducing the number of sick days compared to factor of healthy food. 
...
The summary of this variables provides hints to eliminate 'NA' and extreme values. 

```{r eval=TRUE}
summary(brfss2013$htm4)
```
Actual lower and upper cutoff values come from Wikipedia [@wikishort], [@wikitall].

```{r eval=TRUE}
cbrfss2013<-brfss2013 %>% filter(!is.na(htm4) & htm4 > 50 & htm4 < 280)
```

For initial check of the distribution of heights let us plot its histogram addint the distribution's mean and median lines on top of it.
...
We observe that the distribution is in fact unimodal and seemingly symmetric with the mean and median lines relatively close to each other. We can use a boxplot chat for a further check of symmetry
...
We observe a good match of the probability percentile within 3 standard deviations from the mean and an approximate match for probabilities within 1 and 2 standard deviations  from the mean. The most sensitive measure of 'normality' is a normal probability plot. 
...
The plotted chart suggests a good linear approximation within 3 standard deviations from the mean. Further than 3 standard deviations the chart shows a presence of more data points in the distribution's tails compared to the theoretical normal distribution. 
