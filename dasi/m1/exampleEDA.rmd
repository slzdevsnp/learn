...
The following code snippet shows if people like NZ First more with their age

```{r relationship_between_age_like_nzfirst}
selected_nzes2011 %>%
   filter( !is.na(retiredage))  %>%
   group_by(retiredage) %>% 
   summarise(median_like=median(numlikenzf,na.rm=T) )

```

Revisiting the first question

Add a new variable to separate voters whoh beleive that vote has an influence
...
Showing the contingency table on voters who belive in vote influence and whether they have voted in their
favourite party

```{r votinflu_contingency}
xtabs(~sameparty+votinflu, data=selected_nzes2011)
```
As we can see the conlclusion is that  blah...
