...
#split the dataset roughly in half
train <- sample( c(TRUE,FALSE), nrow(Hitters), rep=TRUE)
length(train)
sum(train)  # how main elmements in train

#test is the complement of train
test <- !train
...
mean.cv.errors <- apply(cv.errors, MARGIN=2, mean) #apply func mean on index 2 (columns)
str(mean.cv.errors)
...
#print(predict(ridge.mod, s=0, exact=TRUE, type="coefficients")[1:20,] ) #produces error
print(predict(ridge.mod, s=0, type="coefficients")[1:20,] )
```
We observe that coefficients are very similar. 
...
Same fuctions are uses as with Ridge except the parameter `alpha=1`
```{r}
...
library(pls)
set.seed(2)
pcr.fit=pcr(Salary~., data=Hitters ,scale=TRUE,validation ="CV") #scale=T normalizes predictors
